...
class TransmitterCardNotification extends StatefulWidget {
  ...
  @override
  _TransmitterCardNotificationState createState() =>
      _TransmitterCardNotificationState();
}

class _TransmitterCardNotificationState
    extends State<TransmitterCardNotification>
  ...
  try {
    final data = await TransmitterAPI().fetchCheckDetailsNotification(docNo, docType);
...
Future<void> _fetchFileNameAndPath() async {
  try {
    final data = await TransmitterAPI().fetchFileNameAndPathNotification(
